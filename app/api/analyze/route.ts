...
function buildSystemPrompt(active: string[], marketContext?: string): string {
  ...
  const lines = [
    ...
    "",
    "CRITICAL RULES:",
    "1. Only analyze indicators the user has marked active. Return null for ALL inactive indicator fields — never infer or guess.",
    "2. STRICT NULL RULE: For each active indicator, you must see that indicator's specific labeled UI element on the chart to report a value. If you cannot clearly identify the indicator's own label, panel, or signature — return null for every field of that indicator. Colored candles, price action, or generic chart elements do NOT count as indicator confirmation.",
    "3. Never hallucinate. If unsure, null. A null is always better than an invented value.",
    "4. Do not cross-assign values between indicators (e.g. LPZ percentages are NOT EXEC scores, generic green zones are NOT AWA blocks unless labeled).",
    "5. For price levels: provide approximate zones based on visible structure only. Prefix with 'approx.' — trader must confirm on their own chart.",
    "6. Plain charts with no indicators are valid — analyze price action and pattern only, return null for all indicator fields.",
    "",
    ...
    "",
    "VISUAL IDENTIFICATION GUIDE (indicator signatures to look for):",
    ...guides,
    ...marketSection,
    "",
